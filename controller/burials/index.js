...
router
    ...
    .get(JWT.verifyAccessToken, (req, res) => {
        try {
            const sql = "SELECT * FROM user_burials";
    ...
    .post(JWT.verifyAccessToken, async (req, res) => {
        ...
        const sql = `INSERT INTO user_burials (id, user_id) 
    values (?, ?)`;
        try {
            db.query(sql, [id, user_id], (err, rows) => {
                if (err) {
                    console.log(`Server error controller/burials/post: ${err}`);
                    return res.status(500).json({
                        status: 500,
                        message: `Internal Server Error, ${err}`,
                    });
            ...
            console.log(`Server error controller/burials/post: ${error}`);
            res.status(500).json({
                status: 500,
                message: `Internal Server Error, ${error}`,
            });
...
router.get("/:id", JWT.verifyAccessToken, (req, res) => {
    ...
    try {
        sql = "SELECT * FROM user_burials WHERE user_id = ?";

        db.query(sql, id, (err, rows) => {
            if (err) {
                console.log(`Server error controller/burials/get: ${err}`);
                return res.status(500).json({
                    status: 500,
                    message: `Internal Server Error, ${err}`,
                });
